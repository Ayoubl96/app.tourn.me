...
import { fetchStageGroups } from '@/api/tournaments/api';
...
export const StageStandings: React.FC<StageStandingsProps> = ({
  ...
  const {
    ...
    loadStandings,
    recalculateStats
  } = useTournamentStandings(tournament.id.toString(), selectedGroupId);
  ...
  useEffect(() => {
    ...
    loadGroups();
  }, [stage.id, callApi, selectedGroupId]);

  // Load standings when group changes
  useEffect(() => {
    if (selectedGroupId) {
      loadStandings();
    }
  ...
  return (
    <Card>
        ...
        <CardTitle className='flex items-center justify-between'>
          <div>
            ...
            {standings && (
              <div className='mt-1 text-sm font-normal text-muted-foreground'>
                Stage: {stage.name} - Group: {standings.group_name}
              </div>
            )}
          </div>
          <div className='flex gap-2'>
            <Button
              ...
              size='sm'
              onClick={loadStandings}
              disabled={isLoading || isRecalculating || !selectedGroupId}
            >
              <RefreshCw
                className={`mr-2 h-4 w-4 ${isLoading ? 'animate-spin' : ''}`}
              />
      ...
      <CardContent>
          ...
          <div className='mb-6'>
            ...
            >
              <SelectTrigger className='w-full max-w-md'>
                <SelectValue placeholder='Select a group to view standings' />
              </SelectTrigger>
              <SelectContent>
                {groups.map((group) => (
        ...
        {!selectedGroupId && groups.length === 0 && !isLoadingGroups ? (
          <div className='py-8 text-center text-muted-foreground'>
          ...
          <div className='py-8 text-center text-muted-foreground'>
            <p>Select a group to view standings.</p>
          </div>
        ) : isLoading ? (
